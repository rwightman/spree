...
def test_handle_match_disconnect_reconnects_and_logs(tmp_path):
    ...
    state = argparse.Namespace(
        agent=old_agent,
        budget=ActivityBudget(max_wall_seconds=60.0),
        completed=True,
...
def test_handle_match_disconnect_does_not_reconnect_after_wall_budget(tmp_path):
    class FakeAgent:
        def __init__(self) -> None:
            self.closed = False
    ...
    class FakeGym:
        ...
        def connect(self, agent_id, model_metadata=None):
            self.connected.append((agent_id, model_metadata))
            raise AssertionError("an expired reconnect must not be admitted")

    old_agent = FakeAgent()
    state = argparse.Namespace(
        agent=old_agent,
        budget=ActivityBudget(max_wall_seconds=0.0),
        completed=True,
        stop_reason="disconnected",
    )
    participant = MatchParticipantRuntime(
    ...
    assert old_agent.closed is True
    assert gym.connected == []
    assert participant.reconnects == 0
    assert state.completed is True
    assert state.stop_reason == "match_wall_seconds"
    events = [json.loads(line) for line in match_log.read_text(encoding="utf-8").splitlines() if line]
    assert events[-1]["type"] == "participant_reconnect_skipped"
    assert events[-1]["reason"] == "match_wall_seconds"
...
def test_run_activity_reports_unknown_disabled_action(capsys, tmp_path):
    from bbs_gym.cli import run_activity

    args = argparse.Namespace(
        agents_config=None,
        no_agents_config=True,
        agent_id="agent-001",
        provider="scripted",
        scripted_response=['{"action": "wait", "arguments": {}}'],
        model=None,
        ...
        codex_stateful=False,
        claude_stateful=False,
        disabled_actions=["hangups"],
        log_path=str(tmp_path / "steps.jsonl"),
    )

    exit_code = run_activity(args)

    captured = capsys.readouterr()
    assert exit_code == 2
    assert "unknown disabled action(s): hangups" in captured.err
    assert "Traceback" not in captured.err


def _match_args(tmp_path, **overrides):
    args = argparse.Namespace(
        match_config=None,
        participant=[],
        agent_id=[],
...
def test_validate_match_args_rejects_wrong_typed_config_values(tmp_path):
    import pytest

    args = _match_args(tmp_path, max_rounds="50")

    with pytest.raises(ValueError, match="max_rounds must be an integer"):
        build_match_scheduler_config(args)


def test_validate_match_args_rejects_non_finite_numbers(tmp_path):
    import pytest

    for field, value in (("max_wall_seconds", float("nan")), ("reconnect_delay", float("inf"))):
        args = _match_args(tmp_path, **{field: value})
        with pytest.raises(ValueError, match=f"{field} must be finite"):
            build_match_scheduler_config(args)


def test_participant_stateful_uses_provider_from_registry(tmp_path):
    registry = AgentRegistry(
        agents={
            "codex-001": AgentRecord(
                agent_id="codex-001",
            ...
            "scripted-001": AgentRecord(
                agent_id="scripted-001",
                bbs_alias="ScriptedOne",
                model={"provider": "scripted"},
            ),
        }
    )
    specs = [
        MatchParticipantSpec(agent_id="codex-001", config={"stateful": True}),
        MatchParticipantSpec(agent_id="scripted-001"),
    ]

    participants = build_match_participants(_match_args(tmp_path), specs, registry)

    # The provider lives only in the registry record; stateful must still apply.
    assert isinstance(participants[0].model, CodexCliAdapter)
    assert participants[0].model.stateful is True
    assert participants[0].runner.profile.prompt_mode == "stateful_delta"


def test_match_agent_ids_colliding_log_paths_rejected(tmp_path):
    import pytest

    args = _match_args(tmp_path, participant=["agent 1:scripted:unused", "agent_1:scripted:unused"])

    with pytest.raises(ValueError, match="log path"):
        build_match_participants(args, match_participant_specs(args), registry=None)


def test_accounts_provision_restricts_payload_permissions(monkeypatch, tmp_path):
    import json as json_module
    import types

    from bbs_gym.cli import accounts_provision
    ...
    agents_path.write_text(
        json_module.dumps(
            {
                "agents": [
                    {"agent_id": "agent-001", "bbs_alias": "AgentOne", "bbs_password": "secret"},
                ]
            }
        ),
        encoding="utf-8",
    ...
    monkeypatch.setattr(
        "bbs_gym.cli.subprocess.run",
        lambda *_args, **_kwargs: types.SimpleNamespace(returncode=0),
    )
    args = argparse.Namespace(agents_config=str(agents_path), dry_run=False, compose_service="bbs")

    exit_code = accounts_provision(args)

    payload_path = tmp_path / "runtime" / "tmp" / "sbbs_agents.resolved.json"
    assert exit_code == 0
    assert payload_path.exists()
    assert payload_path.stat().st_mode & 0o777 == 0o600


def test_build_activity_profile_rejects_invalid_recent_steps_override(tmp_path):
    import pytest

    args = _match_args(tmp_path, agent_id="agent-001", recent_steps_to_keep=-1)

    with pytest.raises(ValueError, match="recent_steps_to_keep"):
        build_activity_profile(args, registry=None)
