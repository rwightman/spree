...
class BbsGym:
    ...
    def connect(
        ...
        if active_transport == "telnet" and record is not None:
            if telnet_password is None:
                raise AccountConfigError(f"telnet login requires a resolved BBS password for {agent_id!r}")
            login_observation = self._login_telnet(observer, session, record.bbs_alias, telnet_password)
    ...
    def _login_telnet(
        ...
        observation = observer.observe_turn(timeout=_TELNET_LOGIN_TIMEOUT, stable_ms=_TELNET_LOGIN_STABLE_MS)
...
def _login_outcome(observation: Observation) -> dict[str, object]:
    return {
        "ready_reason": observation.ready_reason,
        "stable_ms": observation.stable_ms,
        "byte_quiet_ms": observation.byte_quiet_ms,
        "model_text_tail": observation.model_text[-1000:],
    }
